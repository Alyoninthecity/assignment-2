...
/** Creare la classe RomanPrinter e definire la funzione printAsciiArt*/
...
public class RomanPrinter {
    ...
    public static StringBuilder[] printAsciiArt(String romanNumber) {
        StringBuilder[] risultato = new StringBuilder[6];
        RomanPrinter.initS(risultato);
        int lenRomano = romanNumber.length();
        if (lenRomano > 0) {
            ...
            try {
                for (int i = 0; i < lenRomano; i++) {
                    validNumber = false;
                    for (int j = 0; j < 7; j++) {
                        if (romanNumber.charAt(i) == simboli.charAt(j)) {
                            validNumber = true;
                        }
                    }
                    if (!validNumber) {
